...
describe("isMD5", () => {
  it("checks whther a given value is MD5 or not", () => {
    expect(isMD5("874058e8d8582bf85c115ce319c5b0af")).to.equal(true);
    expect(isMD5("874058e8d8582bf85c115ce319c5b0a")).to.equal(false);
  });
});

describe("isSHA1", () => {
  it("checks whther a given value is SHA256 or not", () => {
    expect(isSHA1("a89ca560e2f0f3b081a3e7d0d91a2bcd4e2bdadb")).to.equal(true);
  });
});
...
describe("isASN", () => {
  it("checks whther a given value is ANS or not", () => {
    expect(isASN("ASN13335")).to.equal(true);
  });
});

describe("isDomain", () => {
  it("checks whther a given value is Domain or not", () => {
    expect(isDomain("example.com")).to.equal(true);
  });
});

describe("isIPv4", () => {
  it("checks whther a given value is IPv4 or not", () => {
    expect(isIPv4("8.8.8.8")).to.equal(true);
  });
});

describe("isIPv6", () => {
  it("checks whther a given value is IPv6 or not", () => {
    expect(isIPv6("fe80::d544:e71:3c17:7710%11")).to.equal(true);
  });
});

describe("isEmail", () => {
  it("checks whther a given value is Email or not", () => {
    expect(isEmail("test@test.com")).to.equal(true);
  });
});

describe("isURL", () => {
  it("checks whther a given value is URL or not", () => {
    expect(isURL("https://www.example.com/foo/bar?baz=1")).to.equal(true);
    expect(isURL("https://111.111.111.111/foo/bar?baz=1")).to.equal(true);
  });
});

describe("isCVE", () => {
  it("checks whther a given value is CVE or not", () => {
    expect(isCVE("CVE-1800-0000")).to.equal(false);
    expect(isCVE("CVE-2016-0000")).to.equal(true);
    expect(isCVE("CVE-2100-0000")).to.equal(false);
    expect(isCVE("CVE-2016-00000")).to.equal(true);
    expect(isCVE("CVE-20100-0000")).to.equal(false);
  });
});

describe("isBTC", () => {
  it("checks whther a given value is BTC or not", () => {
    expect(isBTC("1J6PYEzr4CUoGbnXrELyHszoTSz3wCsCaj")).to.equal(true);
  });
});
...
describe("isGATrackID", () => {
  it("checks whther a given value is GATrackID or not", () => {
    expect(isGATrackID("UA-26296840-4")).to.equal(true);
    expect(isGATrackID("UA-26296840")).to.equal(true);
  });
});

describe("isGATrackID", () => {
  it("checks whther a given value is GAPubID or not", () => {
    expect(isGAPubID("pub-9107453047749393")).to.equal(true);
  });
});
